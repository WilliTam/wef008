...
function generate() {
    ...
    for (let y = 0; y < rows; y++) {
      ...
      for (let i of [-1, 0, 1]) {
        for (let j of [-1, 0, 1]) {
          ...
          for (let k = 0; k < candidateName.length; k++) {
            neighbors[k] +=
              currentBoard[(x + i + columns) % columns][(y + j + rows) % rows][k] ;
          }
        }
      }
      ...
      for (let k = 0; k < candidateName.length; k++) {
        if (
          currentState[x][y][k] == 1 &&
          neighbors[k] < survivalMin.value
        ) {
          // Die of Loneliness
          nextState[x][y][k] = 0;
        } else if (
          currentState[x][y][k] == 1 &&
          neighbors[k] > survivalMax.value
        ) {
          // Die of Overpopulation
          nextState[x][y][k] = 0;
        } else if (
          currentState[x][y][k] != 1 &&
          neighbors[k] <= reproductionMax.value &&
          neighbors[k] >= reproductionMin.value
        ) {
          // New life due to Reproduction
          nextState[x][y][k] = 1;
        } else {
          // Stasis
          nextState[x][y][k] = currentState[x][y][k];
        }
      }
    }
  }
  ...
  for (let x = 0; x < columns; x++) {
    for (let y = 0; y < rows; y++) {
      for (let k = 0; k < candidateName.length; k++) {
        if (currentState[x][y][k] == 1) {
          currentBoard[x][y] = candidateName[k];
        }
      }
    }
  }
...
function mouseDragged() {
  ...
  if (currentBoard[x][y] != candidateName[0]) {
    currentState[x][y][0] = 1;
    currentBoard[x][y] = candidateName[0];
